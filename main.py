...
def main():
    ...
    log("Commands: /maintenance /resume /status /stop /help")
    log("=" * 55)
    ...
    while True:
        try:
            ...
            if system_state.shutdown:
                log("SHUTDOWN command received -- closing positions...")
                if open_trades:
                    for trade in open_trades:
                        place_market_sell(trade.ticker,
                            trade.quantity_remaining if trade.tp1_hit else trade.quantity,
                            side=trade.side)
                    close_all_positions()
                    open_trades.clear()
                log("System stopped by /stop command.")
                break
            ...
            if is_pre_market_time() and not _pre_market_done:
                ...
                if not risk_manager.can_trade():
                    ...
                    # نشغّل روتين ما قبل الافتتاح فوراً
