...
# تحديد مسار المشروع حتى يجد Python كل الملفات
...
def _scan_for_signals():
    ...
    if len(meanrev_open) >= 3 and conservative_open:
        return
    ...
    if not conservative_open:
        for signal in results["conservative"]:
            ...
            if trade:
                ...
                found_signal = True
                notify_trade_open(
                    ticker=signal.ticker, strategy="المحافظة",
                    side="BUY", price=signal.entry_price,
                    quantity=trade.quantity, stop_loss=signal.stop_loss,
                    target=signal.target, risk_amount=trade.risk_amount,
                )
                break
