...
# تتبع تشغيل الروتينات اليومية (لتجنب التكرار)
_pre_market_done   = {"date": "", "done": False}
_market_close_done = {"date": "", "done": False}
...
def setup_schedule():
    # نستخدم توقيت نيويورك صراحةً بدلاً من توقيت الخادم
    schedule.every(5).minutes.do(scheduled_runner)
    log("✅ تم إعداد الجدول الزمني (توقيت نيويورك)")
    log("   08:30 AM NY — اختيار الأسهم + تحديث فلترة الأداء")
    log("   09:30 AM NY — افتتاح السوق")
    log("   كل 5 دقائق — فحص الأسهم ومراقبة الصفقات")
    log("   04:05 PM NY — إغلاق الصفقات والتقرير اليومي")


def scheduled_runner():
    """
    يعمل كل 5 دقائق ويقرر ما يجب فعله بناءً على توقيت نيويورك الحقيقي.
    هذا يتجاوز مشكلة schedule الذي يعمل بتوقيت UTC.
    """
    global _pre_market_done, _market_close_done
    ...
    # يعمل فقط أيام الاثنين - الجمعة
    if weekday >= 5:
        return

    # ── إعادة ضبط العلامات عند بداية يوم جديد
    if date_str != _pre_market_done.get("date"):
        _pre_market_done["date"] = date_str
        _pre_market_done["done"] = False
        _market_close_done["date"] = date_str
        _market_close_done["done"] = False

    # ── روتين ما قبل الافتتاح: 8:30 AM نيويورك
    if hour == 8 and 30 <= minute < 35 and not _pre_market_done["done"]:
        _pre_market_done["done"] = True
        pre_market_routine()
        return

    # ── روتين إغلاق السوق: 4:05 PM نيويورك
    if hour == 16 and 5 <= minute < 10 and not _market_close_done["done"]:
        _market_close_done["done"] = True
        market_close_routine()
        return

    # ── فحص عادي خلال ساعات التداول
    scan_routine()
